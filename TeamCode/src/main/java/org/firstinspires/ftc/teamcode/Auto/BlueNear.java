...
//                .splineToConstantHeading(new Vector2d(x_yellow_preload_center,y_yellow_preload_center),Math.toRadians(0))
        ...
        TrajectorySequence COLLECT_CYCLE_2_RIGHT = drive.trajectorySequenceBuilder(yellowRight)
                ...
                .splineToLinearHeading(new Pose2d(15, 13.5, Math.toRadians(180)), Math.toRadians(180),
                        SampleMecanumDrive.getVelocityConstraint(40, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                        SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL))
                .splineToSplineHeading(new Pose2d(-15, 13.5, Math.toRadians(180)), Math.toRadians(180),
                        SampleMecanumDrive.getVelocityConstraint(55, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                        SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL))
                .splineToSplineHeading(new Pose2d(-35, 13.5, Math.toRadians(180)), Math.toRadians(180),
...
//                .splineToSplineHeading(new Pose2d(-22.5, 7, Math.toRadians(168)), Math.toRadians(180))
                ...
                .splineToSplineHeading(new Pose2d(-27.3, 12.2, Math.toRadians(164)), Math.toRadians(180),
                        SampleMecanumDrive.getVelocityConstraint(40, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                        SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL))
                .build();
...
//        calibrator = new DistanceSensorCalibrator(rawReadings, actualDistances);
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            switch (status) {
                ...
                case PURPLE:
                {
                    if(drive.getPoseEstimate().getY() < 41.5 || !drive.isBusy())
                    {
...
//                    double calibratedDistance = calibrator.calibrate(rawReading);
                    ...
                    if(drive.getPoseEstimate().getX() > 44.5)
...
//                }
                ...
                case SCORE_CYCLE:
                {
                    if( redFarAutoController.CurrentStatus == RedFarAutoController.autoControllerStatus.SCORE_CYCLE_DONE && (drive.getPoseEstimate().getX() > 45.1 || !drive.isBusy()))
                    {
