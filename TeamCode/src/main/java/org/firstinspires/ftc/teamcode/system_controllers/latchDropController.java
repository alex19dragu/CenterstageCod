...
public class latchDropController {
    ...
    public void update(robotMap r, latchRightController latchRight, latchLeftController latchLeft, clawAngleController clawAngle) {
        if (CS != PS || CS == INITIALIZE || CS == DROP_ONE || CS == DROP_ONE_RIGHT1 || CS == DROP_ONE_LEFT1 || CS == DROP_ONE_RIGHT2 || CS == DROP_ONE_LEFT2 || CS == DROP_BOTH || CS == DROP_DONE) {
            switch (CS) {

                case DROP_ONE: {
                    if (clawAngle.clawAngle_i >= 4) {
                        if (ok == FALSE) {
                            CS = DROP_ONE_LEFT1;
                        } else {
                            CS = DROP_ONE_LEFT2;
